...
const SYNONYM_GROUPS: SynonymGroup[] = [
  ...
  },
  {
    ...
    terms: ["restaurant", "restaurang", "restauranger", "café", "cafe", "kafé", "bistro", "eatery", "matställe"],
  },
  ...
  { canonical: "law", terms: ["law", "lawyer", "advokat", "advokatbyrå", "juridik", "jurist", "attorney", "legal"] },
  {
    ...
    terms: ["accounting", "accountant", "revisor", "redovisning", "bokföring", "ekonomi", "bookkeeping", "cpa"],
  },
  {
  ...
  {
    canonical: "medical",
    terms: ["medical", "clinic", "klinik", "läkare", "doktor", "hälsocenter", "healthcare", "vård"],
  },
  {
    canonical: "hotel",
    terms: ["hotel", "hotell", "hostel", "vandrarhem", "bed and breakfast", "bnb", "boende", "logi"],
  },
...
/**
 * Returns variant query prefixes to expand search coverage for any location.
 * Google Places treats these as distinct queries and returns different slices
 * of its index — giving us more unique results without hardcoding city zones.
 * Works universally for any city or country.
 */
export function getSearchVariants(niche: string, location: string): Array<{ query: string; location: string }> {
  const q = niche.trim();
  const loc = location.trim();
  if (!loc) return [];

  // Each variant phrase causes Google to return a different result set.
  // Keep to 3 variants max to avoid excessive API usage.
  return [
    { query: `${q} i ${loc}`, location: loc },
    { query: `bästa ${q} ${loc}`, location: loc },
    { query: `${q} nära ${loc}`, location: loc },
  ];
}
