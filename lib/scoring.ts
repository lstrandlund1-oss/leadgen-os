...
function opportunityFromSignals(input: {
  ...
  const proof =
    (input.reviews >= 200 ? 20 :
     input.reviews >= 100 ? 14 :
     input.reviews >= 30 ? 8 :
     input.reviews >= 10 ? 4 : 0) +
    (input.rating >= 4.6 ? 10 :
     input.rating >= 4.3 ? 7 :
     input.rating >= 4.0 ? 4 : 0);
...
function isOperationallyUnstable(input: {
  ...
  if (input.rating > 0 && input.rating < 3.6 && input.reviews < 25)
    return true;
  return false;
}
...
export function scoreLead(
  ...
  const score = clamp(
    round(opportunity * 0.55 + readiness * 0.35 - risk * 0.2 + fitBoost + confBoost),
  );
