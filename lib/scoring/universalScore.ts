...
export function computeUniversalScore(input: UniversalScoreInput): UniversalScoreOutput {
  const {
    reviews, rating, hasWebsite, socialPresence, riskProfile,
    fitScore, gapType, classificationConfidence,
  } = input;
  ...
  let gapPoints = 0;
  switch (gapType) {
    case "INFRASTRUCTURE": gapPoints = 40; break; // No website — clearest gap
    case "CONVERSION":     gapPoints = 32; break; // Has site, no booking system
    case "VISIBILITY":     gapPoints = 22; break; // Weak social/SEO
    case "OPTIMIZATION":   gapPoints = 16; break; // Has everything — still a real gap
  }
  ...
  switch (riskProfile) {
    case "solo_run":             approachPoints = 9;  break;
    case "growing_business":     approachPoints = 8;  break;
    case "independent_business": approachPoints = 7;  break;
    case "local_authority":      approachPoints = 6;  break;
    case "well_established":     approachPoints = 4;  break;
    case "limited_data":         approachPoints = 5;  break;
    case "early_stage":          approachPoints = 3;  break;
    case "unknown":              approachPoints = 5;  break;
    default:                     approachPoints = 5;  break;
  }
  ...
  switch (riskProfile) {
    case "limited_data":         riskBase = 65; break;
    case "early_stage":          riskBase = 55; break;
    case "well_established":     riskBase = 50; break;
    case "local_authority":      riskBase = 42; break;
    case "growing_business":     riskBase = 28; break;
    case "solo_run":             riskBase = 20; break;
    case "independent_business": riskBase = 30; break;
    case "unknown":              riskBase = 45; break;
    default:                     riskBase = 38; break;
  }
  ...
  const riskTooltip = [
    ...
    rating > 0 && rating < 3.5 ? `Below-average rating (${rating.toFixed(1)}★) signals instability.` : "",
    !hasWebsite && socialPresence === "low" ? "No website or social presence increases outreach friction." : "",
  ].filter(Boolean).join(" ");
  ...
  const readinessTooltip = [
    ...
    rating >= 4.2 ? `Strong rating (${rating.toFixed(1)}★) indicates stable operation.` : rating > 0 ? `Rating ${rating.toFixed(1)}★.` : "No rating available.",
  ...
  const valueTooltip = [
    ...
    evidenceLevel === "insufficient" ? "⚠ Low confidence — limited signal data available." :
    evidenceLevel === "low" ? "Some signal data is thin — treat with caution." : "",
  ].filter(Boolean).join(" ");

  // ── BREAKDOWN (for signals tab bars) ─────────────────────────────────────
  const reputation = clamp(round(
  ...
  const digitalPresence = clamp(round(
    (hasWebsite ? 55 : 0) +
    (socialPresence === "high" ? 45 : socialPresence === "medium" ? 25 : 5)
  ));

  const evidenceConfidence = clamp(round(
  ...
  const businessStrength = clamp(round(
  ...
  return {
    ...
    tooltips: {
      ...
      fit: input.fitTooltip ?? `Fit ${fitClamped} — based on how well your capabilities match this lead's detected needs. Open the lead detail to see matched and missing capabilities.`,
      risk: riskTooltip,
