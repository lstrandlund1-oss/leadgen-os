...
export function computeUniversalScore(input: UniversalScoreInput): UniversalScoreOutput {
  ...
  let gapPoints = 0;
  switch (gapType) {
    case "INFRASTRUCTURE": gapPoints = 38; break; // No website — clearest gap
    case "CONVERSION":     gapPoints = 30; break; // Has site, no booking system
    case "VISIBILITY":     gapPoints = 20; break; // Weak social/SEO
    case "OPTIMIZATION":   gapPoints = 8;  break; // Has everything — hardest sell
  }
  ...
  else if (rating > 0 && rating < 3.5) proofPoints = Math.max(0, proofPoints - 3);
  ...
  switch (riskProfile) {
    ...
    case "early_stage":          riskBase = 58; break;
    case "well_established":     riskBase = 52; break;
    case "local_authority":      riskBase = 45; break;
    case "growing_business":     riskBase = 30; break;
    case "solo_run":             riskBase = 22; break;
    case "independent_business": riskBase = 32; break;
    case "unknown":              riskBase = 48; break;
    default:                     riskBase = 40; break;
  }
  ...
  const valueTooltip = [
    `Lead Score ${value} —`,
    `Opportunity ${opportunity} × 0.35,`,
    `Fit ${fitClamped} × 0.30,`,
    `Readiness ${readiness} × 0.20,`,
    `Risk (inverted ${100 - risk}) × 0.15.`,
    evidenceLevel === "insufficient" ? "⚠ Low confidence — limited signal data." :
    evidenceLevel === "low" ? "Limited signal data — treat with some caution." : "",
  ].filter(Boolean).join(" ");
  ...
  return {
    ...
    tooltips: {
      ...
      fit: input.fitTooltip ?? `Fit ${fitClamped} — see fit breakdown above.`,
      risk: riskTooltip,
