...
import { PROFILE_TYPE_DEFINITIONS, type ProfileTypeKey } from "@/lib/profile/profileTypes";
...
export default function ProfileOverviewPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const [profileRes, outcomesRes] = await Promise.all([fetch("/api/profile"), fetch("/api/outcomes?all=true")]);
  ...
  const profileDef = profile ? (PROFILE_TYPE_DEFINITIONS[profile.profileType as ProfileTypeKey] ?? null) : null;
  ...
  const saveNote = (leadId: string, note: string) => {
    ...
    try {
      localStorage.setItem("vantio_lead_notes_v1", JSON.stringify(updated));
    } catch {
      /* ignore */
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-[#f5f0e8] overflow-x-hidden">
            ...
            <section className="rounded-2xl border border-[#1a1a1a] bg-[#0d0d0d] p-6">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-4">
                  <div
                    className="w-12 h-12 rounded-xl border border-[rgba(201,168,76,0.2)] bg-[rgba(201,168,76,0.05)] flex items-center justify-center text-[#c9a84c] text-xl font-light"
                    style={{ fontFamily: "var(--font-display), serif" }}>
                ...
                <div className="mt-5 pt-4 border-t border-[#141414]">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {activeCapabilities.map((cap) => (
                      <span
                        key={cap}
                        className="text-[11px] px-2.5 py-1 rounded-lg border border-[#1e1e1e] bg-[#111] text-[#888]">
              ...
              <section className="rounded-2xl border border-[#1a1a1a] bg-[#0d0d0d] p-6 space-y-4">
                ...
                <div className="space-y-2">
                  {[
                    ...
                    {
                      done: !!(stats && stats.contacted > 0),
                      label: "Contact a lead",
                      sub: "Reach out and log it as contacted in the dashboard",
                      href: "/dashboard",
                    },
                    {
                      done: !!(stats && stats.booked > 0),
                      label: "Book a call",
                      sub: "Mark a reply as booked to track your calendar",
                      href: "/dashboard",
                    },
                    {
                      done: !!(stats && stats.closed > 0),
                      label: "Close your first deal",
                      sub: "Mark a lead as closed and log the revenue",
                      href: "/dashboard",
                    },
                  ].map(({ done, label, sub, href }) => (
                    <div
                      key={label}
                      className={`flex items-start gap-3 rounded-lg px-3 py-2.5 border transition-colors ${done ? "border-[#1a1a1a] opacity-50" : "border-[#252525] bg-[#111]"}`}>
                      <div
                        className={`mt-0.5 w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 ${done ? "border-[#4ade80] bg-[#4ade80]/10" : "border-[#333]"}`}>
                        {done && <span className="text-[9px] text-[#4ade80]">✓</span>}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p
                          className={`text-[12px] font-medium ${done ? "line-through text-[#444]" : "text-[#c8c0b0]"}`}>
                          {label}
                        </p>
                      ...
                      {!done && (
                        <a
                          href={href}
                          className="shrink-0 text-[11px] text-[#c9a84c] hover:text-[#e8c97a] transition-colors mt-0.5">
                          Go →
                        </a>
                      )}
                    </div>
                ...
                <>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {[
                      ...
                      {
                        label: "Reply rate",
                        value: conversionRate(stats.replied, stats.contacted),
                        sub: "of contacted",
                      },
                      {
                        label: "Total revenue",
                        value: stats.totalRevenue > 0 ? fmtRevenue(stats.totalRevenue) : "—",
                        sub: `${stats.closed} deal${stats.closed !== 1 ? "s" : ""} closed`,
                      },
                      {
                        label: "Avg deal size",
                        value:
                  ...
                  <div className="space-y-2">
                    ...
                    {[
                      {
                        label: "Contact → Reply",
                        yours: conversionRate(stats.replied, stats.contacted),
                        bench: "15–25%",
                        raw: stats.contacted > 0 ? stats.replied / stats.contacted : null,
                      },
                      {
                        label: "Reply → Booked",
                        yours: conversionRate(stats.booked, stats.replied),
                        bench: "30–50%",
                        raw: stats.replied > 0 ? stats.booked / stats.replied : null,
                      },
                      {
                        label: "Booked → Closed",
                        yours: conversionRate(stats.closed, stats.booked),
                        bench: "50–70%",
                        raw: stats.booked > 0 ? stats.closed / stats.booked : null,
                      },
                      {
                        label: "Overall close",
                        yours: conversionRate(stats.closed, stats.contacted),
                        bench: "5–15%",
                        raw: stats.contacted > 0 ? stats.closed / stats.contacted : null,
                      },
                      ...
                      return (
                        <div
                          key={label}
                          className="flex items-center justify-between gap-2 rounded-lg border border-[#151515] bg-[#080808] px-3 py-2.5">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            {raw !== null && (
                              <span
                                ...
                                style={{ color: c, border: `1px solid ${c}30`, background: `${c}0a` }}>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="rounded-xl border border-[rgba(201,168,76,0.15)] bg-[rgba(201,168,76,0.03)] p-4 text-center space-y-1">
                      ...
                      <p className="text-[10px] text-[#555]">
                        {stats.closed} closed deal{stats.closed !== 1 ? "s" : ""}
                      </p>
                    </div>
                  ...
                  {savedLeads.map((lead) => {
                    ...
                    return (
                      <button
                        ...
                        {hasNotes && (
                          <div
                            className="shrink-0 w-5 h-5 flex items-center justify-center text-[#8a6e30]"
                            title="Has notes">
                            <svg
                              width="12"
                              height="12"
                        ...
                        {/* Arrow */}
                        <svg
                          className="shrink-0 text-[#333] group-hover:text-[#555] transition-colors"
                          width="14"
                          height="14"
                          viewBox="0 0 24 24"
                          fill="none"
      ...
      {sidePanelLead && (
        <>
          {/* Backdrop */}
          <div className="fixed inset-0 bg-black/60 z-40 backdrop-blur-sm" onClick={() => setSidePanelLead(null)} />
          ...
          <div className="fixed right-0 top-0 h-full w-full max-w-md bg-[#0a0a0a] border-l border-[#1a1a1a] z-50 flex flex-col overflow-hidden">
            ...
            <div className="flex items-start justify-between p-6 border-b border-[#141414]">
              <div className="flex-1 min-w-0 pr-4">
                ...
                <h3
                  className="text-[18px] font-medium text-[#f5f0e8] leading-tight"
                  style={{ fontFamily: "var(--font-display), serif" }}>
                  {sidePanelLead.name}
                </h3>
                <p className="text-[12px] text-[#555] mt-0.5">
                  ...
                  {sidePanelLead.country && sidePanelLead.country !== sidePanelLead.city
                    ? `, ${sidePanelLead.country}`
                    : ""}
                </p>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-5">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  {
                    label: "Score",
                    value: sidePanelLead.score ?? 0,
                    color:
                      (sidePanelLead.score ?? 0) >= 65
                        ? "#4ade80"
                        : (sidePanelLead.score ?? 0) >= 40
                          ? "#c9a84c"
                          : "#f87171",
                    suffix: "/100",
                  },
                  ...
                  {
                    label: "Risk",
                    value: sidePanelLead.risk ?? 0,
                    color:
                      (sidePanelLead.risk ?? 0) >= 60
                        ? "#f87171"
                        : (sidePanelLead.risk ?? 0) >= 30
                          ? "#c9a84c"
                          : "#4ade80",
                    suffix: "/100",
                  },
                    ...
                    <div className="flex-1 bg-[#141414] rounded-full h-1.5">
                      <div
                        ...
                        style={{
                          ...
                          backgroundColor:
                            sidePanelLead.fitScore >= 65
                              ? "#4ade80"
                              : sidePanelLead.fitScore >= 40
                                ? "#c9a84c"
                                : "#f87171",
                        }}
                      />
                    </div>
                    <span className="text-[13px] font-bold text-[#f5f0e8] tabular-nums w-8 text-right">
                      {sidePanelLead.fitScore}
                    </span>
                  </div>
                  {sidePanelLead.matchedNeeds?.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      {sidePanelLead.matchedNeeds.map((n) => (
                        <span
                          key={n}
                          className="text-[10px] px-2 py-0.5 rounded-md border border-[#c9a84c]/20 bg-[rgba(201,168,76,0.05)] text-[#8a6e30]">
                          {n}
                        </span>
                      ))}
                    </div>
              ...
              {sidePanelLead.website && (
                <a
                  href={
                    sidePanelLead.website.startsWith("http")
                      ? sidePanelLead.website
                      : `https://${sidePanelLead.website}`
                  }
                  target="_blank"
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="text-[12px] text-[#666] group-hover:text-[#c9a84c] transition-colors">
                      Visit website
                    </span>
                  </div>
                  <svg
                    className="text-[#333] group-hover:text-[#8a6e30] transition-colors"
                    width="12"
                    height="12"
                    viewBox="0 0 24 24"
                    fill="none"
