...
export default function SettingsPage() {
      ...
      .then(
        (d: {
          ...
          if (d.profile) {
            ...
            if (d.profile.profileType)
              setProfileType((d.profile.profileType as ProfileTypeKey) ?? "performance_marketer");
          }
  ...
  const toggleClass = (on: boolean) =>
    `relative inline-flex h-5 w-9 items-center rounded-full border transition-colors cursor-pointer ${on ? "bg-[rgba(201,168,76,0.2)] border-[#c9a84c]" : "bg-[#111] border-[#252525]"}`;
