  ...
  try {
    ...
    if (!isMobile || MOBILE_NEBULA_ENABLED) {
      const renderAllNebulas = () => {
        ...
        // single one can't affect any other.
      ...
      // All 8 sections render up front, deferred two animation frames for
      // the layout-timing reason above. (A previous version of this lazy-
      // rendered everything past the hero via IntersectionObserver, only
      // drawing each section's nebula once scrolled near it, to cut the
      // main-thread cost on load. That's real savings, but it meant the
      // page visibly had no nebula at all past the hero until scrolled —
      // easy to read as broken rather than "not loaded yet." The
      // downscale-factor increase above (2.5 to 4.5) already cuts the
      // actual computation by ~69% with no visual difference, which is a
      // meaningful win on its own without that trade-off.)
      scopedRAF(() => {
