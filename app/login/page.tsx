...
function LoginForm() {
  ...
  async function handleSubmit() {
    ...
    if (mode === "signup") {
      ...
      else { setPendingEmail(email); setSuccess("Check your email to confirm your account, then come back to sign in."); }
    } else {
    ...
    <div className="w-full max-w-sm">
      <div className="mb-8 text-center">
        ...
        <h1 className="text-3xl md:text-4xl font-light" style={{ fontFamily: "var(--font-display), serif" }}>
          {mode === "signin" ? (<>Sign <span className="italic" style={{ color: "#c9a84c" }}>in</span></>) : (<>Create your <span className="italic" style={{ color: "#c9a84c" }}>account</span></>)}
        </h1>
      </div>
      ...
      <div className="rounded-2xl border border-[#252525] bg-[#111] p-6 space-y-4">
        ...
        {error && <div className="px-4 py-3 rounded-lg border border-rose-500/30 bg-rose-500/5 text-[12px] text-rose-400">{error}</div>}
        {success && (
          <div className="px-4 py-3 rounded-lg border border-emerald-500/30 bg-emerald-500/5 text-[12px] text-emerald-400 space-y-2">
            <p>{success}</p>
            {pendingEmail && !resent && (
              <p className="text-[11px] text-[#555]">Didn&apos;t get it?{" "}
                <button type="button" onClick={resendConfirmation} disabled={resending} className="text-[#c9a84c] hover:text-[#e8c97a] transition-colors disabled:opacity-50">
        ...
        {mode === "signin" && (
          <div className="flex justify-end -mt-1">
            <Link href="/forgot-password" className="text-[11px] text-[#555] hover:text-[#c9a84c] transition-colors">Forgot password?</Link>
          </div>
        )}
      ...
      <p className="text-center text-[12px] text-[#555] mt-6">
        {mode === "signin" ? (
          <>Don&apos;t have an account?{" "}<button type="button" onClick={() => switchMode("signup")} className="text-[#c9a84c] hover:text-[#e8c97a] transition-colors">Sign up</button></>
        ) : (
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-[#f5f0e8] flex flex-col">
      <nav className="flex items-center justify-between px-6 md:px-12 py-5 border-b border-[#252525]">
        ...
        <Link href="/" className="text-[12px] text-[#666] hover:text-[#888] transition-colors">← Back to home</Link>
      </nav>
