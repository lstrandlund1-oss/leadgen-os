...
type Collection = { id: string; name: string; description: string | null; color: string; created_at: string; lead_collection_items?: [{ count: number }] };
type CollectionItem = { id: string; collection_id: string; lead_id: string; company_name: string | null; notes: string | null; added_at: string };
...
export default function CollectionsPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/collections").then(r => r.json()).then((d: { collections?: Collection[] }) => { setCollections(d.collections ?? []); }).finally(() => setLoading(false));
  }, []);
  ...
  async function createCollection() {
    ...
    setSaving(true);
    const res = await fetch("/api/collections", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: newName.trim(), color: newColor }) });
    const d = await res.json() as { collection?: Collection };
    if (d.collection) { setCollections(prev => [d.collection!, ...prev]); setNewName(""); setCreating(false); }
    setSaving(false);
  }
  ...
  async function removeItem(id: string) {
    await fetch("/api/collections/items", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) });
      ...
      <div className="max-w-5xl mx-auto px-5 py-10">
        <div className="flex items-end justify-between mb-8">
          <div>
            ...
            <h1 className="text-3xl md:text-4xl font-light" style={{ fontFamily: "var(--font-display), serif" }}>
              Lead <span className="italic" style={{ color: "#c9a84c" }}>Collections</span>
            </h1>
            <p className="text-[12px] text-[#444] mt-1.5">Group leads into named lists — by client, campaign, or niche</p>
          </div>
        ...
        {creating && (
          <div className="rounded-2xl border border-[rgba(201,168,76,0.3)] bg-[rgba(201,168,76,0.04)] p-5 mb-6 space-y-4">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={createCollection} disabled={!newName.trim() || saving}
          ...
          <div className="space-y-2">
            {loading ? (
              ...
              collections.map(c => {
                ...
                return (
                  <div key={c.id} className={"rounded-xl border transition-all cursor-pointer group " + (isActive ? "border-[rgba(201,168,76,0.4)] bg-[rgba(201,168,76,0.06)]" : "border-[#1a1a1a] bg-[#0d0d0d] hover:border-[#252525]")}
                    onClick={() => loadItems(c)}>
                    <div className="flex items-center gap-3 p-3.5">
                      <div className="w-3 h-3 rounded-full flex-shrink-0" style={{ backgroundColor: c.color }} />
                      <div className="flex-1 min-w-0">
                        <p className={"text-[13px] font-medium truncate " + (isActive ? "text-[#e8c97a]" : "text-[#c8c0b0]")}>{c.name}</p>
                        <p className="text-[10px] text-[#444]">{count} lead{count !== 1 ? "s" : ""}</p>
                      </div>
                      <button type="button" onClick={e => { e.stopPropagation(); deleteCollection(c.id); }}
                        className="opacity-0 group-hover:opacity-100 text-[#333] hover:text-[#f87171] transition-all text-sm">✕</button>
                    </div>
                  </div>
                );
              })
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="text-[11px] text-[#444]">{items.length} lead{items.length !== 1 ? "s" : ""}</span>
                  </div>
                </div>
                {items.length === 0 ? (
                  <div className="rounded-2xl border border-[#1a1a1a] bg-[#0d0d0d] p-10 text-center space-y-2">
                    ...
                    <p className="text-[11px] text-[#333]">Open a lead in the dashboard and click &ldquo;Add to collection&rdquo;</p>
                    <Link href="/dashboard" className="inline-block mt-1 text-[12px] text-[#c9a84c] hover:text-[#e8c97a] transition-colors">Go to Dashboard →</Link>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {items.map(item => (
                      <div key={item.id} className="flex items-center gap-3 rounded-xl border border-[#1a1a1a] bg-[#0d0d0d] px-4 py-3 group">
                        <div className="flex-1 min-w-0">
                          <p className="text-[13px] font-medium text-[#c8c0b0] truncate">{item.company_name ?? item.lead_id}</p>
                          {item.notes && <p className="text-[11px] text-[#444] mt-0.5 truncate">{item.notes}</p>}
                          <p className="text-[10px] text-[#333] mt-0.5">{new Date(item.added_at).toLocaleDateString()}</p>
                        </div>
                        <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <Link href="/dashboard" className="text-[11px] text-[#555] hover:text-[#c9a84c] transition-colors">Open →</Link>
                          <button type="button" onClick={() => removeItem(item.id)} className="text-[#333] hover:text-[#f87171] transition-colors text-sm">✕</button>
                        </div>
                      </div>
                    ))}
                  </div>
